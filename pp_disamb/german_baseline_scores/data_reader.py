...
def read_candidates(parts):
    ...
    for i in range(7, len(parts), 11):
        ...
        can_scores.append([float(s) for s in parts[i + 5:i + 10]])
        can_flags.append(int(parts[i + 10]))
    return can_words, can_tags, can_tfs, can_abs_dists, can_rel_dists, can_scores, can_flags
...
def read_tuba_dz(fp):
    with open(fp, 'r') as f:
        ...
        for line in f:
            ...
            can_words, can_tags, can_tfs, can_abs_dists, can_rel_dists, can_scores, can_flags = read_candidates(parts)
            ...
            all_can_scores.append(can_scores)
