...
import pp_disamb.german_baseline_scores.data_reader as data_reader
import pp_disamb.german_baseline_scores.data_writer as data_writer
from pp_disamb.data.dataset import Dataset, SimpleBatch
from pp_disamb.german_baseline_scores.data_converter import DataConverter
from pp_disamb.german_baseline_scores.data_loader import DataLoader
from pp_disamb.german_baseline_scores.evaluator import Stats
from pp_disamb.utils.embeddings import read_embeddings
...
class Experiment(object):
    ...
    def restore_data_model(self):
        ...
        if self.args.word_emb_file is not None:
            ...
            words, word_embeddings = list(), np.zeros((1, self.args.word_dim))
        if self.args.tag_emb_file is not None:
            tags, tag_embeddings = read_embeddings(self.args.tag_emb_file, mode='txt')
        else:
            tags, tag_embeddings = list(), np.zeros((1, self.args.tag_dim))
    ...
    def init_data_model(self, raw_data):
        ...
        self.args.no_topological_fields = len(self.data_loader.topological_field_vocab)

        print('No. words:        %d' % self.args.no_words)
        print('No. tags:         %d' % self.args.no_tags)
        print('No. topo. fields: %d' % self.args.no_topological_fields)

        self.save_data_model()

    def load_network_class(self, module_name):
        package_name = 'pp_disamb.german_baseline_scores'
        module_name = '.' + module_name
        class_name = 'PPDisambiguator'
        cls = getattr(importlib.import_module(module_name, package_name), class_name)
        return cls

    def init_network(self, session):
        cls = self.load_network_class(self.args.backend)
        self.network = cls(self.args)
        self.network.initialize_global_variables(session)
    ...
    def __train(self, train_dataset, dev_dataset, interval,
        ...
        for vars in batches:
            ...
            self.train_stats.update(loss, end - start, vars[-1], outputs['rounded'], outputs['prediction'])

            if self.iteration % interval == 0:
                self.__validate(dev_dataset, eval_batch_size, session)

    def __validate(self, dev_dataset, eval_batch_size, session):
        batches = dev_dataset.get_batches(eval_batch_size, shuffle=False)
        for vars in batches:
            start = time.time()
            outputs, loss = self.network.eval(vars, session)
            end = time.time()
            self.dev_stats.update(loss, end - start, vars[-1], outputs['rounded'], outputs['prediction'])
        ...
        summary = self.network.make_train_summary(session, stats['train_clf_acc'], stats['train_att_acc'])
        self.summary_writer.add_summary(summary, global_step=self.iteration)
        summary = self.network.make_dev_summary(session, stats['dev_loss'], stats['dev_clf_acc'], stats['dev_att_acc'])
        self.summary_writer.add_summary(summary, global_step=self.iteration)

        self.save()
        self.save_network(session)

        if stats['dev_att_acc'] > self.best_score:
            self.best_score = stats['dev_att_acc']
            self.best_iteration = self.iteration
            self.save_best_network(session)
            print('Save the best model')
        else:
            print('Best score was %.3f at iteration %d' % (self.best_score, self.best_iteration))
...
def train(args):
    ...
    train_raw_data = data_reader.read_tuba_dz(args.train_file)
    print('Load data from file %s, no. sentences: %d' % (args.train_file, len(train_raw_data['sentence_nums'])))

    dev_raw_data = data_reader.read_tuba_dz(args.dev_file)
    print('Load data from file %s, no. sentences: %d' % (args.dev_file, len(dev_raw_data['sentence_nums'])))

    if args.test_file is not None:
        test_raw_data = data_reader.read_tuba_dz(args.test_file)
        print('Load data from file %s, no. sentences: %d' % (args.test_file, len(test_raw_data['sentence_nums'])))
    else:
        test_raw_data = None
    ...
    with tf.Session(config=config) as session:
        if args.continue_training:
            ...
            print('Current iteration: %5d' % experiment.iteration)
            print('Current epoch    : %5d' % experiment.epoch)
            print('Best iteration   : %5d' % experiment.best_iteration)
            print('Best score       : %5.3f' % experiment.best_score)
        else:
...
def evaluate(args):
    ...
    dev_raw_data = data_reader.read_tuba_dz(args.test_file)
    print('Load data from file %s, no. sentences: %d' % (args.test_file, len(dev_raw_data['sentence_nums'])))
    ...
    if args.output_file is not None:
        data_writer.write_tuba_dz(args.output_file, dev_raw_data, results)
...
if __name__ == '__main__':
    ...
    parser_train.add_argument('--backend', type=str, default='network_cross_entropy',
                              help='network module name')

    # Lexicon
    group = parser_train.add_argument_group('lexicon')
    group.add_argument('--word_cutoff', type=int, default=-1,
    ...
    group.add_argument('--eval_batch_size', type=int, default=512,
                       help='mini-batch size in number of sentences')
    ...
    group.add_argument('--word_dim', type=int, default=50,
                       help='word embedding dimension')
    group.add_argument('--tag_dim', type=int, default=50,
    ...
    group.add_argument('--input_dropout', type=float, default=0.2,
    ...
    group.add_argument('--max_norm', type=float, default=None,
                       help='max norm')
    group.add_argument('--batch_normalization', type=str2bool, default=True,
                       help='batch normalization')

    parser_train.add_argument('--debug', type=str2bool, default=False,
                              help='variables and layers summary')
    ...
    parser_eval.add_argument('--batch_size', type=int, default=512,
    ...
    parser_infer.add_argument('--output_file', type=str, default=None,
                              help='output file')
    parser_infer.add_argument('--dir', type=str, default='experiment',
                              help='folder of the model')
